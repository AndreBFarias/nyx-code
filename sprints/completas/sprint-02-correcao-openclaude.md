# Sprint 2: Correção OpenClaude + Ollama

**Objetivo:** Fazer o openclaude funcionar de forma estável com o Ollama local
...
### Sintoma
- A interface do openclaude carrega corretamente (modelo, endpoint exibidos)
...
2. **Tool calling com modelo 3b:**
   O openclaude envia todas as tools do Claude Code (Bash, FileRead, FileEdit, etc.)
...
5. **Variáveis de ambiente persistidas:**
   O openclaude pode estar lendo configurações salvas de sessões anteriores
...
- Rodar openclaude com `--debug --debug-file /tmp/nyx_debug.log`
...
Reproduzir exatamente a request que o openclaude faz:
```bash
...
### 2.3 Configuração do OpenClaude para Ollama
Opções a explorar:
...
- Ou: criar um proxy HTTP entre o openclaude e o Ollama que traduz campos incompatíveis
...
```
openclaude -> proxy Python (porta 11436) -> Ollama (porta 11435)
```
O proxy:
...
- Retorna resposta para o openclaude

### 2.6 Configuração de timeout
- Aumentar timeout do fetch no openclaude (se configurável)
...
- Garantir que o openclaude usa apenas as variáveis de ambiente definidas no run.sh
