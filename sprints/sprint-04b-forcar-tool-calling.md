...
openclaude e a API nativa do Ollama.
...
O `--thinking disabled` do openclaude não propaga `think=false` para o Ollama.
...
Criar um proxy Python leve que fica entre o openclaude e o Ollama:

```
openclaude (porta 11436) → proxy Python → Ollama (porta 11435)
```
...
```bash
# Iniciar proxy antes do openclaude
./venv/bin/python nyx/proxy.py --port 11436 &
PROXY_PID=$!

# Apontar openclaude para o proxy
...
- `run.sh` — iniciar proxy + apontar openclaude
