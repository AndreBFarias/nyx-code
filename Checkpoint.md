...
- **Fase atual:** **Onda 28 em curso (2026-05-18).** 28-06 CONCLUIDA: banner de boot virou block ASCII `$ nyx.code▌` + box `╭─╮│╰╯` com 3 linhas info (v1.2.0 + ● 100% offline em verde; Modelo/Projeto/Rede; Tools/Comandos/Memória/Tipo). Reescrita cirúrgica de `_build_wide()` e `_build_compact()` em `nyx/agent/banner.py` com largura interna fixa = 66 chars (linhas alinhadas em 70 visíveis). Paleta turquesa+roxo+verde preservada (NÃO migrou para roxo claro `#c8b4ff/#9b87ff` do mockup); cursor `▌` roxo, bordas accent turquesa, `100% offline` verde `NYX_SUCCESS`. Assinatura de `build_banner()` intacta — caller em `nyx/cli.py:522` não tocado. Imports novos só do `nyx.themes.design_tokens` (ANSI_PURPLE_FG, ANSI_PRIMARY_FG, ANSI_SUCCESS_FG). Visual confirmado via screenshot real do kitty `/tmp/nyx_tui_banner_28_06_20260518T231443.png`. **Anterior 28-05 CONCLUIDA**: first-run wizard de 7 passos.
- **Último commit (push ok):** `<pendente este commit>` (TUI-REDESIGN-28-06). Anterior: `5fb0035 feat(TUI-REDESIGN-28-05)`.
- **Próxima ação:** TUI-REDESIGN-28-07 (próxima sprint da onda 28).
