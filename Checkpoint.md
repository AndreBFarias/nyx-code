...
- **Fase atual:** **Onda 28 em curso (2026-05-18).** 28-08 CONCLUIDA em 4 commits atômicos (28_08a..d) com 28_08c_PARTE_2 DEFERIDA (switch runtime Application/PromptSession). Arquivo novo `nyx/agent/repl_app.py` (505L) instancia Application full-screen com HSplit (output_buffer + separator + input_buffer + toolbar). KeyBindings espelham PromptSession (Enter/Ctrl+J/Ctrl+O/Ctrl+Up/Tab/Shift+Tab/Ctrl+V/slash). `build_app()` retorna tupla (app, output_buffer, input_buffer); `run_repl_app_async()` envelope com semântica prompt_async. Self-test no-tty validado. `output.py` ganhou `_emit/_eprint/set_repl_app_output/clear_repl_app_output` para routing entre stdout (legacy/headless) e output_buffer (Application). 8 render_* migrados para `_eprint` (render_assistant_start/end, render_user_input + _render_user_soft_box, render_tool_card_start/end, render_thinking_block, render_compaction_event, render_session_stats_card + _render_stats_inline). Flag `app_state['repl_app_active']=False` default em cli.py mantém path PromptSession 100% funcional. Ativação Application em runtime requer reescrita do loop REPL — deferida em SPRINT_TUI_REDESIGN_28_08c_PARTE_2.md. **Anterior 28-07 CONCLUIDA**: cursor blink async.
- **Último commit (push ok):** `<a definir>`. Anteriores: `9125ecc feat(TUI-REDESIGN-28-08c)`, `ace4a03 feat(TUI-REDESIGN-28-08b)`, `022ecc1 feat(TUI-REDESIGN-28-08a)`.
- **Próxima ação:** opcionalmente 28-08c-PARTE-2 (ativação runtime do switch Application/PromptSession) ou mover para próxima onda.
