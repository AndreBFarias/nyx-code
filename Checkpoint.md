...
- **Fase atual:** **Onda 28 segue (2026-05-19).** 28-08c-PARTE-2 CONCLUIDA — switch runtime Application/PromptSession ativo em TTY (NYX_LEGACY_REPL=1 cai em PromptSession). Em TTY, `build_app()` chamado uma vez antes do `while True`; `set_repl_app_output(output_buffer, app_state)` ativa routing global de `_emit` para o buffer da Application; `app_state["repl_app_active"]=True`. Pre-popula banner no buffer com strip ANSI (mitigação imediata: BufferControl não interpreta cores). Loop principal usa `await repl_app.run_async()` por iteração reaproveitando o mesmo `Application` (reset interno via `self.reset()` em cada `run_async`); `input_buffer.text=""` entre iterações. `_flush_buffer` e clears ANSI condicionais ao `repl_app_active` (mantém legacy intacto). Screenshot kitty mostra input ancorado no rodapé + banner topo + bottom toolbar + separador verde — `dev-journey/07-reports/proofs/TUI_28_08c_PARTE_2/input_fixo_rodape_VALIDADO.png` (sha256 `0e266597...`). Achado colateral materializado: TUI-REDESIGN-28-08c-PARTE-3 (substituir BufferControl por FormattedTextControl para renderizar ANSI). **Anterior 28-09 CONCLUIDA**.
- **Último commit:** próximo será `feat(TUI-REDESIGN-28-08c-PARTE-2)`. Anteriores: `6e2597f feat(TUI-REDESIGN-28-09)`, `2187b3f chore(MASTER)`, `339fea5 chore(TUI-REDESIGN-28-09)`.
- **Próxima ação:** TUI-REDESIGN-28-08c-PARTE-3 (render ANSI no output_buffer) ou continuar 12 sprints anti-débito em `producao/`.
