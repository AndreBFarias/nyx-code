...
- **Fase atual:** **Onda 28 segue (2026-05-19).** 28-08c-PARTE-3 CONCLUIDA — `output_control` agora é `FormattedTextControl(text=lambda: ANSI(output_buffer.text))`; `output_buffer` (Buffer) segue como armazenamento canônico append-only (compatível com `append_to_buffer` e `set_repl_app_output`); auto-scroll via `get_vertical_scroll=_scroll_to_bottom` no Window. Em `nyx/cli.py`, pre-populate do banner volta a usar string ANSI bruta (sem strip). Banner renderiza COM CORES: bordas turquesa, `nyx.code` roxo + cursor de blink, `100% offline` verde, `MEMÓRIA` com acento, toolbar turquesa dim. Captura kitty 130x30: `dev-journey/07-reports/proofs/TUI_28_08c_PARTE_3/banner_com_ansi_VALIDADO.png` (sha256 `97f6ef34...e1052e`). Smoke + invariantes 14/14 + smoke legacy + self-test + gauntlet rápido (7/7, 15.4s) OK. **Anteriores 28-08c-PARTE-2 e 28-09 CONCLUIDAS**.
- **Último commit:** próximo será `feat(TUI-REDESIGN-28-08c-PARTE-3)`. Anteriores: `f1504f8 feat(TUI-REDESIGN-28-08c-PARTE-2)`, `6e2597f feat(TUI-REDESIGN-28-09)`, `2187b3f chore(MASTER)`.
- **Próxima ação:** continuar 12 sprints anti-débito em `producao/`.
