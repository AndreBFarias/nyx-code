...
- **Fase atual:** **2026-05-19 — COCKPIT-LIFECYCLE-FIX-01 CONCLUIDA.** 3 níveis defensivos: (1) `inspect_pid_file()` + `PtyBridge.preflight()` em `nyx/cockpit/pty_bridge.py` lê NYX_PID_FILE antes de spawn; (2) handler WS `/repl` em `nyx/cockpit/server.py` retorna `{type:"busy", reason, pid}` em vez de cascade-kill quando PID anterior está vivo; (3) `Lifecycle.acquire()` em `nyx/agent/services/lifecycle.py` aceita env `NYX_LIFECYCLE_TAKEOVER=0` para abortar sem matar. 8 testes sintéticos PASS + 3 cenários WebSocket reais PASS (busy retornado com reason). Smoke + invariantes 14/14 + gauntlet rapido 7/7 APROVADO. Achado colateral: 5 violações de acentuação pré-existentes em `nyx/cockpit/server.py:162,261-264` viraram sprint COCKPIT-ACENTUACAO-FIX-01 (PENDENTE).
- **Anteriores:** 28-08c-PARTE-3 CONCLUIDA — `output_control` agora é `FormattedTextControl(text=lambda: ANSI(output_buffer.text))`; auto-scroll via `get_vertical_scroll=_scroll_to_bottom` no Window. Banner renderiza COM CORES.
- **Último commit:** próximo será `fix(COCKPIT-LIFECYCLE-FIX-01)`. Anteriores: `93cb18f feat(TUI-REDESIGN-28-08c-PARTE-3)`, `f1504f8 feat(TUI-REDESIGN-28-08c-PARTE-2)`, `6e2597f feat(TUI-REDESIGN-28-09)`.
- **Próxima ação:** continuar 11 sprints anti-débito em `producao/` (COCKPIT-LIFECYCLE-FIX-01 saiu da lista, COCKPIT-ACENTUACAO-FIX-01 entrou).
