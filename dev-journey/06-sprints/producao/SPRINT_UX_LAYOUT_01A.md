...
> - ADR-023 Design System: tokens e paleta D em `nyx/themes/design_tokens.py`.
> - ADR-024 Render Layer: `print()` apenas em `cli.py` e `agent/output.py`.
>
...
> - Sprint anterior: UX-DESIGN-01 CONCLUIDA — `design_tokens.py` disponível com `BOX_CHARS`, paleta D, `NYX_ACCENT`, `NYX_MUTED`, `NYX_PURPLE`.
> - UX-LAYOUT-01 original absorvido por UX-LAYOUT-01A (este arquivo) e UX-LAYOUT-01B.
...
- Algum hex ficou em `cli.py` (gambiarra #1 das "burlas de linter").
- Modo compacto foi "implementado" só com um `if cols < 80: pass` (stub como implementação).
- Mexeu em `_bottom_toolbar` ou `render_user_input` (fora de escopo — essas são UX-LAYOUT-01B e TUI-FIX-07B).
- Banner "visual" só foi testado em cols=120 e quebra em cols=40.
- Criou constantes de cor redundantes em `cli.py` ("copy-paste sem adaptação").

Reportar se falhar:
```
[SPRINT UX-LAYOUT-01A] BLOQUEADA: <motivo em 1 linha>
```
