...
**Status:** CONCLUIDA (2026-06-01 — port verificado E describe-real provado. (1) Gauntlet --only vision 3/3 (V-VS-01 importa/instancia; V-VS-02 is_available(); V-VS-03 describe+cache skip), baseline_2026-06-01 total 3/3. (2) moondream instalado via ./install.sh sem flag, NA pasta do projeto (run.sh:247 export OLLAMA_MODELS=$SCRIPT_DIR/models; manifest em models/manifests/.../moondream/latest). (3) Describe-real comprovado: VisionService.describe(nyx_user_FINAL.png) -> transcrição não-vazia e coerente ("computer screen displaying a black and white text-based document with the word 'code' in blue...") em 19.7s; segunda chamada cache-hit 0.0s (VISION-02). (4) is_available()=True via config DEFAULT do app (OLLAMA_URL porta 11435 + project/models). (5) verify_vram delta 0 MiB (moondream CPU puro num_gpu=0, ADR-022). (6) Cleanup: moondream duplicado removido de ~/.ollama (1a tentativa foi p/ models default por env errado do daemon manual), zero daemon órfão, VRAM 64/4096 livre. CAVEAT (ambiental, não código): `ollama serve` manual SEM os guards de OOM do run.sh (nyx-runtime-limits.sh/ulimit/oom_score_adj) caiu (RemoteProtocolError) na 2a inferência numa máquina de 4GB; o app real sobe ollama COM esses guards. Recomendado: confirmar 2 chamadas de visão consecutivas sob `./run.sh` real sem crash. AC#1/#2/#3/#5 satisfeitos.)
