...
| 125ff | **INFRA-RUFF-NOQA-FORMAT-01** | 25.meta Anti-débito de pipeline | BAIXA | PENDENTE (achado IMPORTANTE do validador INFRA-OOM-STATS-CLI-01: 7 warnings ruff `Invalid noqa directive` em scripts/gauntlet/nyx_gauntlet.py linhas 116/237/1240/1245/1261/1269/1283 — todos com `# noqa-acento` que não é sintaxe ruff válida; adicionalmente 2 RUF100 sobre PLC0415/BLE001 não-habilitados no pyproject. Pré-existentes desde commit 8632b5e2 (2026-05-19). Ruff retorna exit 0 — warnings cosméticos, não erros. Fix: trocar `# noqa-acento` por código real (ex: `# noqa: E501` ou similar) ou remover; lembrar que noqa-acento é marker custom do validar-acentuacao.py — talvez melhor solução é manter formato atual e configurar `external = ["noqa-acento"]` em pyproject (já tentado em RUFF-EXTERNAL-NOQA-CONFIG-01 mas insuficiente para silenciar). Touches: scripts/gauntlet/nyx_gauntlet.py 7+2 linhas, talvez pyproject.toml. Out-of-scope: refactor amplo de markers customizados.) | INFRA-OOM-STATS-CLI-01 |
...
| 125jj | **INFRA-PLANEJADOR-ACENTUACAO-AUTO-01** | 25.meta Anti-débito de pipeline | BAIXA | PENDENTE (achado PONTO-CEGO do validador INFRA-OOM-HISTORY-01: spec MD SPRINT_INFRA_OOM_HISTORY_01.md tem 43 violações sistemáticas de acentuação PT-BR (palavras-exemplo entre aspas como exemplificação do problema sem acentos); planejador-sprint deveria emitir specs com acentuação PT-BR correta no template; touches: template do planejador em ~/.claude/agents/planejador-sprint.md ou similar; aceita: spec gerado pelo /planejar-sprint passa validar-acentuacao.py --paths exit 0; alternativa: pós-processamento sed automatizado no fluxo do /sprint-ciclo após geração do spec; BLOQUEADA aguarda autorização do usuário para mexer em agente global fora do repo Nyx) | INFRA-OOM-HISTORY-01 |
