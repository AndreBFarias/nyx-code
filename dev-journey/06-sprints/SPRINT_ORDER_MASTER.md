...
| 125z | **SPEC-FIX-OOM-02-FASE-RB04** | 25.meta Anti-débito de pipeline | BAIXA | PENDENTE (achado MINÚCIA do validador INFRA-OOM-02 — spec arquivado em concluidos/SPRINT_INFRA_OOM_02.md tem 2 inconsistências textuais: (a) linha 53 e 393 referenciam "RB-04 PASS em --only rapido" mas RB-04 vive em robustez_boot (rapido mapeia para {infra,proxy,visual,config}); (b) linha 67 acceptance #6 menciona setattr+toggle mas RB-03 real não usa esse padrão (só getattr+hasattr+grep). Touches: 1 arquivo (spec arquivado), 2 hunks textuais. Out-of-scope: alterar código — RB-04 funciona corretamente em robustez_boot.) | INFRA-OOM-02 |
