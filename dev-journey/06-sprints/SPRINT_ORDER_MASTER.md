...
| 125ii | **INFRA-VALIDATE-ACENTUACAO-CLI-FIX-01** | 25.meta Anti-débito de pipeline | BAIXA | PENDENTE (achado TRIVIAL do validador INFRA-OOM-RETRY-STEP-01: specs/templates usando `validar-acentuacao.py <path>` sem flag `--paths`; comportamento atual: exit 0 mesmo com violações se sem flag; fix: padronizar uso de `--paths` em todos os specs PROOFs-OF-WORK; touches: dev-journey/06-sprints/concluidos/*.md (busca + replace) ou template) | INFRA-OOM-RETRY-STEP-01 |
