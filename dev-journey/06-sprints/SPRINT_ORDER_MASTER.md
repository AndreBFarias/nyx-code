...
| 125kk | **INFRA-SANITIZER-FIX-05** | 25.meta Anti-débito de pipeline | ALTA | PENDENTE (achado da sessão Planejador 2026-05-21: incidente sanitizer histórico de 2026-05-20 23:36 corrompeu 57 arquivos do working tree removendo glifos U+25xx; check #14 do sprint_invariants.sh foi NEUTRALIZADO simultaneamente porque compara literais que também sumiram; vulnerabilidade estrutural: defesa precisa ser imune a auto-mutilação; fix: reescrever check #14 usando chr(0x25CF), chr(0x25CB), chr(0x25D0) em vez de literais; investigação forense confirmou sanitizer atual `~/.config/zsh/scripts/universal-sanitizer.py` (mtime 2026-05-20 19:27 pós SPR-2026-05-20-sanitizer-hardening) PRESERVA via ALLOWED_GLYPHS — vetor inrastreável sem audit trail; restauração `git checkout HEAD -- .` recuperou tudo, smoke OK invariantes 14/14 REAL; touches: scripts/sprint_invariants.sh (chr substituição) + VALIDATOR_BRIEF.md (nova seção [CORE] Defesa anti-sanitizer); acceptance: chr(0xNNNN) no source + invariantes 14/14 + BRIEF seção criada; spec em producao/SPRINT_INFRA_SANITIZER_FIX_05.md) | INFRA-SANITIZER-FIX-04 |
