...
| 125gg | **INFRA-OOM-PATTERNS-KV-CACHE-01** | 24.1 Infra resiliente | BAIXA | PENDENTE (achado IMPORTANTE do validador INFRA-OOM-RETRY-STEP-01: log de runtime 2026-05-20 01:14:59 mostrou "failed to allocate buffer for kv cache" NÃO casando `_is_oom_error` (_OOM_PATTERNS em proxy.py:79-87 não inclui "kv cache" nem "GGML_ASSERT"); resultado: retry intermediário propagou HTTP 500 ao cliente sem cair para fallback CPU; sintoma exposto pela sprint nova mas pré-existente latente; fix: adicionar 2 patterns em nyx/proxy.py:79-87 + atualizar RB-05 para assertar coverage; touches: nyx/proxy.py (~2L); acceptance: log mostrar "kv cache" acionando degradação CPU normalmente; bloqueante para hardware OOM crônico) | INFRA-OOM-RETRY-STEP-01 |
