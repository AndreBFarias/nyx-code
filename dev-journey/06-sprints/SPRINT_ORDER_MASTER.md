...
| 125hh | **GAUNTLET-RB05-CAP-FIX-01** | 25.1 Resiliência do gauntlet | BAIXA | PENDENTE (achado MINÚCIA do validador INFRA-OOM-RETRY-STEP-01: heurística cap-counter do RB-05 no gauntlet conta 1 quando deveria contar 2 retries; RB-05 ainda passa (cap reconhece 1+ retries) mas asserto não distingue 1 vs 2; fix: ajustar contagem para tracking explícito do cap; opcional, pode ser tratado junto com INFRA-OOM-PATTERNS-KV-CACHE-01) | INFRA-OOM-RETRY-STEP-01 |
...
| 125jj | **INFRA-PLANEJADOR-ACENTUACAO-AUTO-01** | 25.meta Anti-débito de pipeline | BAIXA | PENDENTE (achado PONTO-CEGO do validador INFRA-OOM-HISTORY-01: spec MD SPRINT_INFRA_OOM_HISTORY_01.md tem 43 violações sistemáticas de acentuação PT-BR (palavras "nao", "sessao", "execucao", "criacao", "acao", "proximo", "ultimo", "diretorio" sem acentos); planejador-sprint deveria emitir specs com acentuação PT-BR correta no template; touches: template do planejador em ~/.claude/agents/planejador-sprint.md ou similar; aceita: spec gerado pelo /planejar-sprint passa validar-acentuacao.py --paths exit 0; alternativa: pós-processamento sed automatizado no fluxo do /sprint-ciclo após geração do spec) | INFRA-OOM-HISTORY-01 |
