...
| 125r | **MEMORY-INTENT-ENFORCE-01** | 25.0 Release (Infra > Modelo) | ALTA | PENDENTE (anti-débito derivado do gauntlet 22:53 CTX-11 FAIL "tool_calls=0 write_memory=False" — usuário disse "lembra que eu uso pyenv 3.12" e modelo respondeu só texto sem chamar write_memory. Filosofia "infra força modelo a ser bom" exige classifier de intent "save_memory" em nyx/agent/intent.py com padrões linguísticos ("lembra|guarda|registra|memoriza|salva (na memoria|para depois)|anota") → quando detectado, proxy força re-issue com hint "O usuário pediu para você LEMBRAR de algo. Use a tool write_memory(key, value) AGORA." e/ou injeta exemplos few-shot no system prompt; em caso de tool_calls=0 após hint, log WARN. Touches: nyx/agent/intent.py, nyx/proxy.py (path tool-needed), possivelmente nyx/agent/prompt.py (few-shot). Out-of-scope: refactor profundo de ADR-026.) | NYX-PROMPT-REINJECT-01 |
