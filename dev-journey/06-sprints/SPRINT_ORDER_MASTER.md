...
| 125s | **REGISTRY-SUDO-IMPORT-01** | 25.0 Release (Integração ADR-013) | MÉDIA | PENDENTE (anti-débito derivado do gauntlet 22:53 COV-01 FAIL "1 sem import: ['sudo_session']" — ADR-013 (Integração obrigatória) exige TODO arquivo em nyx/agent/tools/ importado em registry.py, INCLUSIVE singletons. sudo_session.py é módulo singleton (não tem classe Tool — state global de senha cacheada do SUDO-MODE-01), mas ADR-013 não tem cláusula de exceção. Fix: adicionar `from nyx.agent.tools import sudo_session  # noqa: F401 -- singleton module-level state, mantém arquivo carregado conforme ADR-013` em nyx/agent/tools/registry.py. Alternativa: catalogar exceção formal em ADR-013 amendado se preferir manter import segregado. Touches: 1 arquivo, 1 linha + nota.) | -- |
| 125t | **SCAFFOLD-CMD-FIX-01** | 25.1 Resiliência do gauntlet | MÉDIA | PENDENTE (anti-débito derivado do gauntlet 22:53 SCF-02 FAIL "[Errno 2] No such file or directory: '/home/andrefarias/Dese..." — scripts/scaffold.py command path provavelmente quebrou após algum refactor recente (commands/ virou pacote em CTX-04 ou anterior). Investigação: rodar `python scripts/scaffold.py command __gauntlet_test_cmd "teste" --category sessao` manualmente, ver erro literal. Fix: ajustar path no scaffold para o novo layout (provavelmente nyx/agent/commands/__init__.py vs nyx/agent/commands.py legacy). Touches: scripts/scaffold.py. Out-of-scope: refactor profundo do scaffold.) | -- |
| 125u | **GAUNTLET-SYNC-02-RECOVER-01** | 25.1 Resiliência do gauntlet | BAIXA | PENDENTE (anti-débito derivado do gauntlet 22:53 SYNC-02 FAIL details vazio — verifica `"tools registradas" in r.stdout.lower() or "Todas" in r.stdout` mas r.stdout não contém nenhuma dessas. Investigação: rodar `python scripts/sync.py` e capturar output literal, comparar com fixture do gauntlet 3619-3621. Fix: atualizar string match para refletir novo output de sync.py OU corrigir sync.py se output regrediu. Touches: scripts/gauntlet/nyx_gauntlet.py e/ou scripts/sync.py. Out-of-scope: refactor do sync.) | -- |
| 125p | **GAUNTLET-IF-05-PLAN-REGRESS-01** | 25.1 Resiliência do gauntlet | BAIXA | PENDENTE (anti-débito derivado do gauntlet completo 22:53 — IF-05 era OK no gauntlet 2148 anterior com output `"Crie um plano de implementação para: feature X"` que continha string `"list_files"` no texto longo; CTX-04 (commit 669f217) transformou `/plan` de "gerador de template de prompt" em "checklist persistida opt-in" — novo output `"Objetivo do plano definido: feature X"` não menciona list_files; fix: atualizar scripts/gauntlet/nyx_gauntlet.py:1730-1732 para verificar nova semântica via `"feature X" in plan` e/ou `"Objetivo" in plan` em vez de `"list_files" in plan`; touches: 1 arquivo, 2 linhas; out-of-scope: reverter CTX-04 — comando funciona corretamente, é o teste que está desatualizado) | CTX-04 |
