...
def _count_services() -> int:
    ...
    if not services_dir.exists():
        return 0
    return sum(
        1 for f in services_dir.glob("*.py")
        if f.stem != "__init__"
    )
...
def _count_sprints() -> dict[str, int]:
    ...
    return {
        ...
        "concluidos": sum(1 for f in (base / "concluidos").glob("SPRINT_*.md")) if (base / "concluidos").exists() else 0,
...
def update_claude_md(tools: int, commands: int, services: int, tests: int,
                     adrs: int, sprints: dict[str, int], check: bool) -> bool:
...
def update_readme(tools: int, commands: int, services: int, tests: int, check: bool) -> bool:
    ...
    replacements = [
        ...
        (r'Tools \| 40 \| \d+ \| \d+%',
         f'Tools | 40 | {tools} | {tools * 100 // 40}%'),
        (r'Commands \| 98 \| \d+ \| \d+%',
         f'Commands | 98 | {commands} | {commands * 100 // 98}%'),
        (r'Services \| 35 \| \d+ \| \d+%',
         f'Services | 35 | {services} | {services * 100 // 35}%'),
        (r'\d+ tools registradas',
         f'{tools} tools registradas'),
        (r'\d+ slash commands',
         f'{commands} slash commands'),
        (r'\d+ testes',
         f'{tests} testes'),
    ]
...
def main() -> None:
    ...
    print(f"  ADRs:      {adrs}")
    print(f"  Sprints:   {sprints['concluidos']} concluídas | {sprints['producao']} pendentes | {sprints['backlog']} backlog")
